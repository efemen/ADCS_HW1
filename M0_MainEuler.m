...
for i = 1:N-1
    w_dot(i, :) = [w1_dot(w(i,2), w(i,3)); w2_dot(w(i,1), w(i,3)); w3_dot(w(i,1), w(i,2))];
    w(i + 1, :) = w(i, :) + w_dot(i, :) * dt;

    e_angles_dot(i, :) = F3_EulerDot(w(i, :), e_angles(i, :));
    e_angles(i + 1, :) = e_angles(i, :) + e_angles_dot(i, :) * dt;
end
...
e_angles_deg = F4_MatrixAngleNormalizerDeg(rad2deg(e_angles));
w_deg_s = rad2deg(w);
...
e_euler_angles = e_angles_deg_raw;

save euler_e_angles.mat e_euler_angles
